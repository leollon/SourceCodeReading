...
class Router:
    def __init__(
        ...
        if lifespan is None:
            ...
            # https://docs.python.org/3.8/library/inspect.html#inspect.isasyncgenfunction
            warnings.warn(
            ...
            # https://docs.python.org/3.8/library/inspect.html#inspect.isgeneratorfunction
            warnings.warn(
    ...
    async def not_found(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        # 如果是在一个 starlette 应用程序内运行，那么抛出异常，这样配置的异常处理程序能够处理返回的响应。
        # 对于普通 ASGI 应用程序，简单返回响应即可。
        if "app" in scope:
    ...
    async def lifespan(self, scope: Scope, receive: Receive, send: Send) -> None:
        """
        处理 ASGI lifespan 消息，这消息使得我们能够管理应用程序的启动以及关闭事件。
        """
        started = False
        app = scope.get("app")
        await receive()
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        for route in self.routes:
            # 判断任意路由(route)匹配是否匹配 incoming scope
            # 如果找到，则将 incoming scope 交给找到的路由。
        ...
        if partial is not None:
            # 处理部分匹配。有某种情况是，某个端点能够处理请求，但不是一个首选的方式。
            # 特别的是，我们用这个来处理 "405 Method Not Allowed"。
